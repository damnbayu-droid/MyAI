/**
 * Register a {@link DeepSeekAdapter} for the `myai` provider route on
 * `ctx.llm`, with connection facts resolved per request instead of frozen at
 * load: the plugin layers its `cordis.yml` entry config under the optional
 * `llm-myai` user-settings section (`ctx.settings`) and resolves the API
 * key through the optional credential seam (`ctx.credentials`), so a changed
 * base URL, catalog, or key reaches the very next request without restarting
 * anything, while an in-flight stream keeps the facts it started with. The
 * one registration-captured fact — the retry policy — re-registers the route
 * in place when it changes.
 * @module @deepseek-ai/dsh-llm-myai
 */
...
export const name = 'llm-myai'
export const inject = ['llm']

const NS = settingsNamespace('llm-myai')
const DEFAULT_API_KEY_ENV = 'MYAI_API_KEY'
...
function resolveModels(models: readonly DeepSeekCatalogModel[] | undefined): DeepSeekCatalogModel[] {
  ...
  return (models ?? DEFAULT_MODELS).map((model) => {
    if (model.id.length === 0) throw new Error('llm-myai: catalog model ids must be non-empty')
    if (model.name !== undefined && model.name.length === 0) {
      throw new Error(`llm-myai: catalog model "${model.id}" has an empty name`)
    }
    if (model.contextWindow !== undefined
      && (!Number.isInteger(model.contextWindow) || model.contextWindow <= 0)) {
      throw new Error(
        `llm-myai: catalog model "${model.id}" contextWindow must be a positive integer`,
      )
    }
    if (model.maxTokens !== undefined
      && (!Number.isInteger(model.maxTokens) || model.maxTokens <= 0)) {
      throw new Error(
        `llm-myai: catalog model "${model.id}" maxTokens must be a positive integer`,
      )
    }
    if (seen.has(model.id)) throw new Error(`llm-myai: duplicate catalog model "${model.id}"`)
    seen.add(model.id)
    return {
      id: model.id,
...
export function resolveAdapterOptions(config: Config, environment?: LaunchEnvironmentSnapshot): ResolvedDeepSeekOptions {
  ...
  if (config.maxTokens !== undefined
    ...
    throw new Error('llm-myai: maxTokens must be a positive safe integer')
  }
  ...
  if (!Number.isFinite(streamIdleTimeoutMs)
    ...
    throw new Error(
      `llm-myai: streamIdleTimeoutMs must be a positive finite number no greater than ${MAX_TIMER_DELAY_MS}`,
    )
  }
  return {
    ...
    streamIdleTimeoutMs,
    retryPolicy: resolveRetryPolicy(config.retryPolicy, 'llm-myai: retryPolicy'),
  }
}
...
export function apply(ctx: Context, config: Config): void {
  ...
  const options = (): ResolvedDeepSeekOptions => {
    ...
    try {
      ...
      lastRaw = raw
      ctx.logger.error('llm-myai: keeping the last good configuration after an invalid settings section')
  ...
  const resolveApiKey = async (connection: ResolvedDeepSeekOptions): Promise<string> => {
    ...
    if (credentials !== undefined) {
      ...
      if (hit !== undefined) return assertUsableApiKey(hit.value, 'llm-myai', ref)
    } else {
      // Without the seam there is no managed store to rank against, so the
      // environment is the whole credential plane.
      const ambient = launchEnvironmentOf(ctx).get(ref)
      if (ambient !== undefined && ambient.value.length > 0) {
        return assertUsableApiKey(ambient.value, 'llm-myai', ref)
      }
    }
    throw new LlmError(
      `llm-myai: no API key for provider route "${PROVIDER}"; store ${ref} through the credentials`
  ...
  ctx.llm.registerConfigurableProviders([
    { provider: PROVIDER, displayName: 'MyAI', settingsNs: NS, settingsPath: [] },
  ])
