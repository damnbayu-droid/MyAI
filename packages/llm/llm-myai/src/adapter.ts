...
export class DeepSeekAdapter extends LlmAdapter {
  ...
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    try {
      ...
      if (timeoutOf(watchdog.signal, STREAM_IDLE_TIMEOUT_CODE) !== undefined) {
        throw new LlmError(
          `MyAI stream idle timeout after ${connection.streamIdleTimeoutMs}ms`,
          'TIMEOUT',
          { cause: error },
        )
      }
      if (options.signal?.aborted) {
        throw new LlmError('MyAI request aborted by caller', 'ABORTED', { cause: error })
      }
      if (error instanceof LlmError) throw error
      throw new LlmError(`MyAI API stream from ${connection.baseURL} failed`, 'TRANSPORT', { cause: error })
    } finally {
      consumer.abort('MyAI stream consumer stopped')
  ...
  private async * request(
    ...
    try {
      ...
      throw new LlmError(
        `MyAI API request to ${connection.baseURL} failed`,
        'TRANSPORT',
        { cause: error },
      )
    }
    ...
    if (!response.ok) {
      let message = `MyAI API error (HTTP ${response.status})`
    ...
    if (!response.body) {
      throw new LlmError('MyAI API returned no response body', 'EMPTY_RESPONSE')
    }
